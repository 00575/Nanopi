...
#cp -f $GITHUB_WORKSPACE/assets/banner package/base-files/files/etc/banner
...
sed -i '/skip\=/ a skip=`oot-mount | grep -q /dev/$device; echo $?`' `find package/ -follow -type f -path */automount/files/15-automount`
